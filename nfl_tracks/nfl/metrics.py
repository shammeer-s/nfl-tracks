...
def calculate_separation(play_data: pd.DataFrame, config: NFLTracksConfig):
    ...
    for frame_id in sorted(play_data[config.frame_col].unique()):
        ...
        # Calculate pairwise distances between all offensive and defensive players
        distances = cdist(offense_df[['x', 'y']], defense_df[['x', 'y']])

        # For each offensive player, find the minimum distance to a defender
...
def get_play_speed_stats(play_data: pd.DataFrame, config: NFLTracksConfig):
    ...
    Returns:
        pd.DataFrame: A DataFrame with max_speed and avg_speed for each player.
    """
    # 's' is the speed column from the dataset description
    ...
    # Merge with player names for more readable outputs
...
def get_total_distance_traveled(play_data: pd.DataFrame, config: NFLTracksConfig):
    ...
    This is computed by summing the Euclidean distance between player coordinates in consecutive frames.

    Returns:
        pd.DataFrame: A DataFrame with the total distance traveled for each player.
    """
    distance_data = []
    for player_id in play_data[config.player_id_col].unique():
        ...
        # Calculate the difference in x and y coordinates between consecutive frames
        diffs = np.diff(player_df[['x', 'y']].values, axis=0)
        # Calculate the Euclidean distance for each step and sum them
